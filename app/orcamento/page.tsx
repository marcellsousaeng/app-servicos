      ...
      <main className="max-w-md mx-auto px-5 pt-6">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white p-3 rounded-xl shadow-lg active:scale-95 transition-all"
          >
          ...
          <div className={`w-full max-w-md rounded-t-[40px] p-6 max-h-[92vh] overflow-y-auto border-t shadow-2xl ${
            ...
            <div className="space-y-4 pb-10">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                    {form.fotos.map((foto, index) => (
                      <div key={index} className="relative min-w-30 h-24 rounded-xl overflow-hidden border border-slate-700">
                        ...
                        <button onClick={() => removerFoto(index)} className="absolute top-1 right-1 bg-blue-500 p-1 rounded-full text-white shadow-lg"><Trash2 size={10} /></button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="space-y-3">
                ...
                <textarea className={`w-full border p-3 rounded-xl text-xs h-20 outline-none focus:border-blue-500 ${clean ? 'bg-slate-50 border-slate-200 text-slate-800' : 'bg-slate-900 border-slate-700 text-white'}`} placeholder="Descrição do Serviço" value={form.descricao} onChange={e => setForm({...form, descricao: e.target.value})} />
                ...
                <textarea className={`w-full border p-3 rounded-xl text-xs h-16 outline-none focus:border-blue-500 ${clean ? 'bg-slate-50 border-slate-200 text-slate-800' : 'bg-slate-900 border-slate-700 text-white'}`} placeholder="Observações Adicionais" value={form.observacao} onChange={e => setForm({...form, observacao: e.target.value})} />
              </div>

              <button onClick={salvarOrcamento} className="w-full py-4 bg-blue-600 text-white rounded-xl font-black uppercase text-sm shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all">
                <Save size={18} />
...
function InputMoeda({ Icone, placeholder, value, onChange, clean }: any) {
  return (
    <div className="relative">
      ...
      <input 
        className={`w-full border py-3 pl-14 pr-3 rounded-xl text-xs outline-none focus:border-blue-500 font-mono ${
...
function InputIcon({ Icone, placeholder, value, onChange, clean }: any) {
  return (
    <div className="relative">
      <Icone size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
      <input 
        className={`w-full border py-3 pl-10 pr-3 rounded-xl text-xs outline-none focus:border-blue-500 ${
            clean ? 'bg-slate-50 border-slate-200 text-slate-800' : 'bg-slate-900 border-slate-700 text-white'
        }`} 
        placeholder={placeholder} 
        value={value} 
