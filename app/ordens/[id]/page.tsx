...
export default function DetalhesOSPage() {
  ...
  const exibirFaturamento = ordem?.status === 'Finalizado' || ordem?.status === 'Em andamento' || ordem?.status === 'Aguardando material'
  ...
  return (
    <div className={`min-h-screen pb-32 transition-colors duration-300 ${clean ? 'bg-slate-50 text-slate-900' : 'bg-[#07111f] text-white'}`}>
      <main className="max-w-md mx-auto px-5 pt-6">
        ...
        {/* DADOS DE FATURAMENTO (AGORA PARA EM ANDAMENTO E FINALIZADO) */}
        {exibirFaturamento && (
          <section className={`rounded-3xl p-6 mb-8 border shadow-lg transition-all ${
            ordem.status === 'Finalizado' ? 'border-emerald-500/20 shadow-emerald-500/5' : 'border-blue-500/20 shadow-blue-500/5'
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={numPedido} onChange={(e) => setNumPedido(e.target.value)} placeholder="Ex: 4500987"
                  className={`w-full rounded-2xl p-4 text-sm font-bold outline-none border transition-all ${clean ? 'bg-slate-50 border-slate-200 focus:border-blue-500' : 'bg-[#111c2e] border-slate-700 focus:border-blue-500'}`} />
              </div>
              <div>
              ...
              <button onClick={salvarDadosFaturamento} disabled={salvandoDadosExtras} 
