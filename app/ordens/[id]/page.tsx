      ...
      <main className="max-w-md mx-auto px-5 pt-6">
        ...
        <div className="flex items-center justify-between gap-3 mb-6">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-black uppercase italic">OS #{ordem.numero_os ?? ordem.id}</h1>
            <span className={`text-[10px] font-black px-2 py-0.5 rounded-md border ${badgeEstilo(ordem.status)}`}>
              {ordem.status}
            </span>
          </div>
          <button onClick={() => setModalAtualizacao(true)} className={`h-12 px-4 rounded-2xl flex items-center gap-2 text-xs font-black uppercase border ${clean ? 'bg-blue-600 border-blue-400 text-white' : 'bg-[#0d1726] border-blue-500/30 text-blue-400'}`}>
        ...
        {/* FOTO DO SERVIÇO (REESTABELECIDA) */}
        <section className={`rounded-3xl p-6 mb-5 border shadow-xl ${clean ? 'bg-white border-slate-100' : 'bg-[#0d1726] border-slate-800'}`}>
        ...
        {/* BOTÃO ACRESCENTAR MATERIAL */}
        <div className="mb-6">
          <button 
            onClick={() => router.push(`/ordens/${id_os}/material`)}
            className={`w-full py-4 rounded-2xl border-2 border-dashed flex items-center justify-center gap-3 active:scale-95 transition-all ${
              clean ? 'bg-white border-blue-500/30 text-blue-600' : 'bg-[#0d1726] border-blue-500/20 text-blue-400'
            }`}
          >
            <div className="w-8 h-8 rounded-full bg-blue-500/10 flex items-center justify-center"><Plus size={20} /></div>
            <span className="font-black uppercase italic">Acrescentar Material</span>
          </button>
        </div>
          ...
          <div className={`w-full max-w-md mx-auto rounded-t-[40px] p-8 pb-10 border-t ${clean ? 'bg-white border-slate-200' : 'bg-[#0d1726] border-slate-700'}`}>
            ...
            <div className="space-y-4">
              <textarea placeholder="O que foi feito?" value={descricaoAtualizacao} onChange={(e) => setDescricaoAtualizacao(e.target.value)}
                className={`w-full rounded-2xl p-4 text-sm font-medium outline-none min-h-[120px] border ${clean ? 'bg-slate-50 border-slate-200' : 'bg-[#111c2e] border-slate-700'}`} />
              <input placeholder="Técnico responsável" value={tecnicosResponsaveis} onChange={(e) => setTecnicosResponsaveis(e.target.value)}
                className={`w-full rounded-2xl p-4 text-sm font-medium outline-none border ${clean ? 'bg-slate-50 border-slate-200' : 'bg-[#111c2e] border-slate-700'}`} />
              <button onClick={salvarAtualizacao} disabled={salvandoAtualizacao} className="w-full bg-blue-600 py-5 rounded-2xl font-black uppercase text-white shadow-lg active:scale-95 disabled:opacity-50">
...
function MenuNav({ titulo, Icone, ativo, clean, onClick }: any) {
  return (
    <button onClick={onClick} className={`flex flex-col items-center justify-center py-2 ${ativo ? 'text-blue-500' : clean ? 'text-slate-400' : 'text-slate-500'}`}>
      ...
      <span className="mt-1 text-[10px] font-bold uppercase">{titulo}</span>
    </button>
  )
}
